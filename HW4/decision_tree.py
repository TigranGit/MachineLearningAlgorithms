...
def calculate_entropy(y):
  ...
  if len(N) == 0 or len(y) == 0:
    ...
    N = np.hstack((0, N)) if values == 0 else np.hstack((N, 0))
  if (N == 0).all():
    return 0
  p = N[1] / (N[0] + N[1])
  if p == 0 or p == 1:
...
class DTClassifier:
  ...
  def calculate_purity_gain(self, y, y1, y2):
    ...
    if len(y1) == len(y2) == 0:
      return 0
    purity_parent = self.impurity(y)
    purity_1 = self.impurity(y1)
    purity_2 = self.impurity(y2)
    s_1 = len(y1)
    s_2 = len(y2)
    pure_gain = purity_parent - (s_1 * purity_1 + s_2 * purity_2) / (s_1 + s_2)
    return pure_gain
